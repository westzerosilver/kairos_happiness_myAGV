...
class ImageSubscriber:
    def __init__(self):
        ...
        self.object_action_done = False
    def extract_keypoints_descriptors_from_image(self):
        img = cv2.imread("/home/uk/catkin_ws/src/agv_control/scripts/tree.jpg")
    ...
    def callback(self, data):
        try:
            ...
            if self.found_person:
            ...
            elif self.detect_object(img) and not self.object_action_done:
                ...
                twist_cmd.linear.y = 0.0
                self.cmd_vel_pub.publish(twist_cmd)
                self.object_action_done = True

            else:
                if total_contour_pixels >= 1105:
                    twist_cmd = Twist()
                    ...
                    if len(filtered_contours_y) == 0:
                        ...
                        twist_cmd = Twist()
                        twist_cmd.linear.x = 0.01  # Adjust linear velocity as needed
                        twist_cmd.angular.z = 0.0  # No angular velocity for straight movement
                        self.cmd_vel_pub.publish(twist_cmd)
